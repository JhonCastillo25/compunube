#!/bin/bash
echo "Instalar LXD"
sudo snap install lxd

echo "agregar usuario"
sudo gpasswd -a vagrant lxd

echo "creando Preseed file"
cat >> lxd2config.yalm << EOF

config:
  core.https_address: 172.42.42.102:8443
networks:
- config:
    bridge.mode: fan
    fan.underlay_subnet: 10.0.2.0/24
  description: ""
  managed: true
  name: lxdfan0
  type: bridge
storage_pools:
- config:
    source: ""
  description: ""
  name: local
  driver: dir
profiles:
- config: {}
  description: ""
  devices: {}
  name: default
cluster:
  server_name: serv2
  enabled: true
  member_config: []
  cluster_address: 172.42.42.101:8443
  cluster_certificate: |
    -----BEGIN CERTIFICATE-----
    
    -----END CERTIFICATE-----
  server_address: ""
  cluster_password: admin
EOF

echo"usar preseed"
cat lxd2config.yaml | sudo lxd init --preseed

echo "crear contenedores "
lxc init ubuntu:18.04 cont1 --target serv2
lxc init ubuntu:18.04 contbk1 --target serv2

echo "iniciar contenedores"
lxc start cont1
lxc start contbk1

echo "instalar apache2"

lxc exec cont1 -- apt-get install apache2 -y
lxc exec contbk1 -- apt-get install apache2 -y

echo "crear index.http"

cat >> index.html<< EOF
Bienvenido al contenedor LXD1
EOF
lxc file push index.html cont1/var/www/html/index.html

cat >> index.html << EOF
Bienvenido al contenedor de backup 1
EOF

lxc file push index.html contbk1/var/www/html/index.html

echo "redireccionamiento de puertos"
lxc config device add cont1 myport80 proxy listen=tcp:172.42.42.102:5080 connect=tcp:127.0.0.1:80
lxc config device add contbk1 myport80 proxy listen=tcp:172.42.42.102:6080 connect=tcp:127.0.0.1:80

echo "reiniciar apache2"
lxc exec cont1 -- systemctl restart apache2
lxc exec contbk1 -- systemctl restart apache2
