#!/bin/bash
echo "Instalar LXD"
sudo snap install lxd

echo "agregar usuario"
sudo gpasswd -a vagrant lxd

echo "creando Preseed file"
cat >> lxd3config.yalm << EOF
config:
  core.https_address: 172.42.42.103:8443
networks:
- config:
    bridge.mode: fan
    fan.underlay_subnet: 10.0.2.0/24
  description: ""
  managed: true
  name: lxdfan0
  type: bridge
storage_pools:
- config:
    source: ""
  description: ""
  name: local
  driver: dir
profiles:
- config: {}
  description: ""
  devices: {}
  name: default
cluster:
  server_name: serv3
  enabled: true
  member_config: []
  cluster_address: 172.42.42.101:8443
  cluster_certificate: |
    -----BEGIN CERTIFICATE-----
    
    -----END CERTIFICATE-----
  server_address: ""
  cluster_password: admin
EOF

echo"usar preseed"
cat lxd3config.yaml | sudo lxd init --preseed

echo "crear contenedores "
lxc init ubuntu:18.04 cont2 --target serv3
lxc init ubuntu:18.04 contbk2 --target serv3

echo "iniciar contenedores"
lxc start cont2
lxc start contbk2

echo "instalar apache2"

lxc exec cont2 -- apt-get install apache2 -y
lxc exec contbk2 -- apt-get install apache2 -y

echo "crear index.http"

cat >> index.html<< EOF
Bienvenido al contenedor LXD2
EOF
lxc file push index.html cont2/var/www/html/index.html

cat >> index.html << EOF
Bienvenido al contenedor de backup 2
EOF

lxc file push index.html contbk2/var/www/html/index.html

echo "redireccionamiento de puertos"
lxc config device add cont1 myport80 proxy listen=tcp:172.42.42.103:5080 connect=tcp:127.0.0.1:80
lxc config device add contbk1 myport80 proxy listen=tcp:172.42.42.103:6080 connect=tcp:127.0.0.1:80

echo "reiniciar apache2"
lxc exec cont2 -- systemctl restart apache2
lxc exec contbk2 -- systemctl restart apache2
